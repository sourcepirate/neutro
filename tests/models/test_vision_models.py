...
def test_vgg16_channels_first_summary_reports_shapes(capsys):
    model = VGG16(input_shape=(3, 32, 32), num_classes=10, data_format='channels_first')
    model.summary()
    assert "multiple" not in capsys.readouterr().out


def test_vgg19_channels_first_summary_reports_shapes(capsys):
    model = VGG19(input_shape=(3, 32, 32), num_classes=10, data_format='channels_first')
    model.summary()
    assert "multiple" not in capsys.readouterr().out
